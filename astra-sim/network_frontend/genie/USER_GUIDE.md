...
```
The in CMAKELists.txt designates how granular (or if not) to capture. 
- NONE: Do not generate any chrometrace file
...
# OR
CHROMETRACE=NONE bash build/astra_genie/build.sh
```
...
```bash
git clone git@github.com:jinsun-yoo/ibintercept.git
make all 
export PATH=
