...
QueuepairManager::QueuepairManager(std::shared_ptr<gloo::transport::Context> context, std::shared_ptr<spdlog::logger> logger, int send_id, int recv_id, int nqps) {
    ...
    for (int qp_idx = 0; qp_idx < nqps; ++qp_idx) {
        ...
        int send_cts_qp_idx = qp_idx + nqps; // Use the second half of QPs for CTS messages in pingpong mode.
        const auto&send_cts_pair = _context->getPair(send_id, send_cts_qp_idx);
        send_cts_pair->setSync(true, true);
        int receive_cts_qp_idx = qp_idx + nqps; // In pingpong mode, the second half of QPs are used for the reverse direction.
        const auto&recv_cts_pair = _context->getPair(recv_id, receive_cts_qp_idx);
...
void QueuepairManager::send_cts_message(int qp_idx, int stream_id) {
    int next_send_idx = send_ctx_next_idx[qp_idx];
    auto* cts_entries = cts_send_ptrs[qp_idx];
    ...
    cts_send_buffers[qp_idx]->send(next_send_idx * CTS_SIZE, CTS_SIZE, next_send_idx * CTS_SIZE, -1);
    send_ctx_next_idx[qp_idx] = (next_send_idx + 1) % NUM_CTS_ENTRY;
}

void QueuepairManager::poll_send_cts_complete(int qp_idx) {
    cts_send_buffers[qp_idx]->pollQP();
}

int QueuepairManager::check_incoming_cts(int qp_idx) {
    int next_recv_idx = recv_ctx_next_idx[qp_idx];
    auto* cts_entries = cts_recv_ptrs[qp_idx];
    ...
    if (marked_stream_id != -1) {
        ...
        recv_ctx_next_idx[qp_idx] = (next_recv_idx + 1) % NUM_CTS_ENTRY;
