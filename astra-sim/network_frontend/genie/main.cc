...
// test_ctx->gloo_context = backingContext;
    ...
    if (AstraSim::env_var_is_set("GENIE_ONLY_SCALEOUT")) {
        std::cout << "GENIE_ONLY_SCALEOUT is set" << std::endl;
        if (args.num_npus != SCALE_UP_GROUP_SIZE) {
            std::cerr << "GENIE_ONLY_SCALEOUT is set, but num_npus (" << args.num_npus
                      << ") != SCALE_UP_GROUP_SIZE (" << SCALE_UP_GROUP_SIZE << ")" << std::endl;
            throw std::runtime_error("GENIE_ONLY_SCALEOUT is set, but num_npus != SCALE_UP_GROUP_SIZE");
        }
