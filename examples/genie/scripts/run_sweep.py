    ...
    for size_pow in range (3, 12):
        for iteration in range (10):
            ...
            with open(filename, 'r') as f:
                content = f.read()
                matches = re.findall(r'finished, (\d+) cycles', content)
                if matches:
                    cycles_list = []
                    sum_num = 0
                    for m in matches:
                        cycles_list.append(int(m)/1000_000)
                        sum_num += int(m)/1000_000
                    average = sum_num / len(cycles_list)
                    print(f"Size: {coll_size}, Iter: {iteration}, Cycles: {cycles_list}, Average: {average}")
...
def get_line_data(log_dirname: str):
    ...
    for size_pow in range (3, 12):
        ...
        with open(filename, 'r') as f:
            content = f.read()
            pattern = r'^\s*\d+\s+\d+\s+\w+\s+\w+\s+[-\d]+\s+(\d+(?:\.\d+)?)'
            ...
            if matches:
                ...
                for m in matches:
                    nccl_multirun_data.append(float(m) / 1000)
                print(f"Size: {coll_size}, Cycles: {cycles_list}, Average: {m}")
            else:
...
def draw_plot(config: PlotConfig):
    ...
    ax.set_ylabel('Duration (ms)', fontsize=18)
